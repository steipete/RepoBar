...
- Issue Navigator window for pasted GitHub URLs, Actions run URLs, commit URLs and hashes, issue/PR references, direct selected-repository issue inputs like `73655` or `gh-42`, unique repo-name shorthands like `discrawl#64`, grouped repo-heading lists like `owner/repo: #1, #2`, same-line repository prose like `owner/repo: PR 18`, numbered repository headings with nested count summaries, line-bounded explicit repository prose series like `owner/repo PR 7 and 8`, mixed explicit repository series like `owner/repo issue #7 and PR #8`, repository-scoped issue inputs like `owner/repo gh-42`, primary URL list shortcuts like `1. #2172` plus an indented GitHub URL for that same reference, leading triage-list references like `- #461/#460` that suppress incidental item-detail refs, single repository-only list context like `1. owner/repo`, copied local project path context, normal prose repository context like `Found in owner/repo`, heading-scoped prose references, contextual back references, compound shorthands, and commit hashes, chained/ranged shorthands like `owner/repo#70/#71` and `owner/repo#66-#69`, and singular or plural prose reference lists like `PR 123, 456 and 789` or `pull requests 123 and 456` with an embedded browser preview, active-host GitHub Enterprise links, and pasted-list order preservation, plus an optional clipboard reference watcher that opens copied references in Issue Navigator from a tray balloon
...
CI uploads unsigned `win-x64` and `win-arm64` publish layouts as `repobar-windows-win-x64-unsigned` and `repobar-windows-win-arm64-unsigned`. Trusted CI pushes to `main` and `v*` tags then run a protected **Windows signing** job in the `release-signing` environment for the `win-x64` installer path. That job uses the same Azure Trusted Signing profile as the OpenClaw Windows node app: `azure/login` with `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, `AZURE_TENANT_ID`, and `AZURE_SUBSCRIPTION_ID`, followed by `azure/trusted-signing-action` against the `hanselman` signing account and `WindowsEdgeLight` certificate profile at `https://wus2.codesigning.azure.net/`. RepoBar signs `RepoBar.Windows.exe`, verifies the Authenticode policy with `Scripts\Test-WindowsReleaseSignatures.ps1`, builds the Inno installer from the signed layout, signs the installer, verifies its Authenticode signature, and uploads `repobar-windows-win-x64-signed` plus `repobar-windows-installer-win-x64-signed`.

Use **Launch at login** in Preferences to add or remove the current executable from the current user's Windows `Run` registry key. Use **Check for updates automatically** to run a silent startup update check and show a tray balloon when a newer Windows installer or release page is available. Use **Check for updates** from the tray menu to compare the running app version with the latest RepoBar GitHub release and open the Windows installer asset that best matches the current process architecture when one is attached. Use **Copy update diagnostics** to copy the current executable path, install directory, app version, OS, process architecture, release API, and Windows installer asset preference for troubleshooting update/install issues.
...
Repository lists are scoped by active account through `repositoriesByAccount`. Existing single-account `repositories` settings are migrated into the active account and kept mirrored for backward compatibility. Account IDs and `repositoriesByAccount` keys are normalized together when settings load, so scriptable settings that use spaces or mixed case still keep the intended active profile. Missing active account IDs fall back to the default profile. GitHub host values may be plain hosts or URLs; any path portion is stripped during normalization. Null or invalid account, repository, owner-filter, and Actions-owner entries from generated JSON are ignored during normalization. Invalid enum-backed preferences such as menu scope, sort, heatmap, terminal, activity, and notification-click values fall back to their defaults. Switching accounts in the tray switches both the credentials and the visible repository list, so GitHub.com, Enterprise, personal, and work profiles do not share pinned or hidden repositories by accident.
...
Use **Customize menu** in Preferences to hide or reorder main tray actions and repository submenu entries, including the local **Open folder**, **Open in terminal**, and **Worktrees** actions plus individual repository **Pin/Unpin**, **Set visible**, **Hide**, **Move up**, and **Move down** controls. Legacy `visibility` customization settings are expanded into those individual manage actions when settings load. Preferences, About, and Quit remain required so a customized menu cannot trap the app without settings, support diagnostics, or exit controls. Use **Clear response cache** to remove the active account's persisted GitHub response bodies and ETags before the next refresh.

Use **Watch clipboard references** to enable the clipboard-only GitHub reference monitor. Windows polls text copied to the clipboard, ignores the initial clipboard contents as a baseline, sends a tray balloon for newly copied issue, pull request, workflow run, or commit references, and opens Issue Navigator with the copied text when the balloon is clicked. Full GitHub URLs keep their source host, while bare `#123` references use the active account host and selected default repository. When copied terminal output includes an exact discovered local project path, bare references inherit that local repository context before falling back to the selected repository.
...
Use **Diagnostics** from the tray menu to inspect and copy Windows runtime state: settings path, active account, repository counts, refresh/update settings, repository scope/sort/limit settings, local Git inventory plus scan/fetch/sync settings, Actions/clipboard/PR-notification settings, active account cache directory and entry count, archive database status, last refresh error, and captured rate-limit buckets. The diagnostics window also exposes forced refresh and active-account cache clearing so Windows can recover the same cache/debug states as the macOS debug pane.
...
.\Scripts\Test-WindowsValidationArtifacts.ps1
```

The smoke publishes the app, creates a local Git fixture for `steipete/RepoBar`, writes a two-account smoke settings file with a smoke archive database, launches `RepoBar.Windows.exe`, verifies the settings file, waits for the app's runtime summary, and writes a JSON proof summary under `dist\windows\smoke\`. The summary records the running process, executable path, active account, scoped credential targets, sample repository, local Git attachment, archive-backed issue/PR fallback rows, refresh/update settings, repository scope/sort/display settings, local scan/fetch/sync settings, Actions/clipboard/PR-notification settings, diagnostics logging state and log-file creation, rendered tray/support menu items, rendered repository submenu items, and screenshot status. `Test-WindowsValidationArtifacts.ps1` verifies the TRX counters, required smoke proof booleans, and required `win-x64`/`win-arm64` publish layouts, then writes `repobar-windows-validation.json` beside the smoke summaries. When a desktop surface is available, the smoke also writes a PNG screenshot next to the JSON summary. The Windows CI job uploads this directory as the `repobar-windows-smoke` artifact and also uploads the compact validation manifest as `repobar-windows-validation`.
...
When `discoverLocalProjects` is enabled, RepoBar scans `localProjectsRoot` for Git checkouts. It matches each checkout's `origin` remote host and owner/name to repositories configured for the active account, then adds branch, upstream, ahead/behind, dirty-file, local branch switching, worktree creation/navigation, fetch, sync, rebase, reset, and folder/terminal actions to the tray menu. `terminalPreference` can be `auto`, `windowsTerminal`, `powerShell`, or `commandPrompt`; auto tries Windows Terminal, then PowerShell, then Command Prompt. When `fetchLocalProjectsBeforeStatus` is true, `localProjectsFetchIntervalMinutes` controls how often automatic status refreshes run `git fetch --prune --quiet` for each local repository. When `showDirtyFilesInMenu` is true, the dirty-file section shows up to three changed file names. Repositories without a local match can be checked out into `localProjectsRoot` from the tray.
