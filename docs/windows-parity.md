...
| Repository submenu | GitHub links, local state, Finder/Terminal open actions, worktrees, issues, PRs, releases, changelog, CI runs, discussions, tags, branches, contributors, commits, activity, recent issue/PR filters, pin/hide, hide/reorder customization with grouped sections | Same actions where GitHub APIs support them; Windows shell opens folders/terminal through independent customizable submenu entries, can check out missing local repos, shows scoped global and per-repository Commit/Activity feeds, filters recent issues by viewer/label and recent PRs by viewer/engagement, and can hide/reorder grouped repository submenu entries with individual pin/visible/hide/move controls | Mostly |
