...
| Repository submenu | GitHub links, local state, worktrees, issues, PRs, releases, changelog, CI runs, discussions, tags, branches, contributors, commits, activity, pin/hide, hide/reorder customization | Same actions where GitHub APIs support them; Windows shell opens folders/terminal, can check out missing local repos, and can hide/reorder repository submenu entries | Mostly |
...
| Repository browser | Search accessible repos, set Visible/Pinned/Hidden, display limit, menu sort, and display/menu customization | Native settings window with filtered accessible repo discovery, visibility controls, display limit, menu sort, and main/repository menu customization | Mostly |
...
| Notifications | Optional PR notifications for new PRs, updates, review requests, and comments with browser or Issue Navigator click handling | Windows tray notifications for new PRs, updates, review requests, and comments with configurable browser or Issue Navigator click-through and persistent duplicate suppression across transient empty refreshes | Mostly |
