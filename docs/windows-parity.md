...
| Repository browser | Search accessible repos, filter forked/archived repos, set Visible/Pinned/Hidden, owner/status filters, display limit, menu sort, and display/menu customization | Native settings window with filtered accessible repo discovery, fork/archive inclusion toggles, visibility controls, owner/status display filters, display limit, menu sort, and main/repository menu customization | Mostly |
