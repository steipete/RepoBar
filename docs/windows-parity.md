...
| Repository submenu | GitHub links, local state, worktrees, issues, PRs, releases, changelog, CI runs, discussions, tags, branches, contributors, commits, activity, recent issue/PR filters, pin/hide, hide/reorder customization | Same actions where GitHub APIs support them; Windows shell opens folders/terminal, can check out missing local repos, shows global and per-repository Activity feeds, filters recent issues by viewer/label and recent PRs by viewer/engagement, and can hide/reorder repository submenu entries | Mostly |
