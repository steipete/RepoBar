...
| Repository submenu | GitHub links, local state, worktrees, issues, PRs, releases, changelog, CI runs, discussions, tags, branches, contributors, commits, activity, recent issue/PR filters, pin/hide, hide/reorder customization | Same actions where GitHub APIs support them; Windows shell opens folders/terminal, can check out missing local repos, filters recent issues by viewer/label and recent PRs by viewer/engagement, and can hide/reorder repository submenu entries | Mostly |
...
| Repository browser | Search accessible repos, filter forked/archived repos, set Visible/Pinned/Hidden, display limit, menu sort, and display/menu customization | Native settings window with filtered accessible repo discovery, fork/archive inclusion toggles, visibility controls, display limit, menu sort, and main/repository menu customization | Mostly |
