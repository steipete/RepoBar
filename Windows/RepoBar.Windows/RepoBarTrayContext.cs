...
internal sealed class RepoBarTrayContext : ApplicationContext
{
    ...
    private void WriteSmokeRuntimeSummary()
    {
        ...
        try
        {
            ...
            var summary = new
            {
                ...
                refreshIntervalMinutes = _settingsStore.Settings.RefreshIntervalMinutes,
                checkForUpdatesAutomatically = _settingsStore.Settings.CheckForUpdatesAutomatically,
                repositoryMenuScope = _settingsStore.Settings.RepositoryMenuScope.ToString().ToLowerInvariant(),
                repositorySortKey = _settingsStore.Settings.RepositorySortKey.ToString().ToLowerInvariant(),
                repositoryDisplayLimit = _settingsStore.Settings.RepositoryDisplayLimit,
                discoverLocalProjects = _settingsStore.Settings.DiscoverLocalProjects,
                localProjectsRoot = _settingsStore.Settings.LocalProjectsRoot,
                localProjectsMaxDepth = _settingsStore.Settings.LocalProjectsMaxDepth,
                fetchLocalProjectsBeforeStatus = _settingsStore.Settings.FetchLocalProjectsBeforeStatus,
                localProjectsFetchIntervalMinutes = _settingsStore.Settings.LocalProjectsFetchIntervalMinutes,
                autoSyncLocalProjects = _settingsStore.Settings.AutoSyncLocalProjects,
                showDirtyFilesInMenu = _settingsStore.Settings.ShowDirtyFilesInMenu,
                ...
                showActionsUsage = _settingsStore.Settings.ShowActionsUsage,
                actionsMonitoredOwners = _settingsStore.Settings.ActionsMonitoredOwners.ToArray(),
                enableGitHubReferenceMonitor = _settingsStore.Settings.EnableGitHubReferenceMonitor,
                enablePullRequestNotifications = _settingsStore.Settings.EnablePullRequestNotifications,
                pullRequestNotificationClickAction = _settingsStore.Settings.PullRequestNotificationClickAction.ToString().ToLowerInvariant(),
    ...
    private void AddActionsUsageItems(ToolStripItemCollection items)
    {
        ...
        if (_actionsInsights.HasData)
        {
            ...
            {
                actions.DropDownItems.Add(new ToolStripMenuItem(
                    $"Plan: {_actionsInsights.PlanTier.DisplayName()} ({_actionsInsights.IncludedMinutesPerMonth:n0} minutes, {_actionsInsights.ConcurrentJobs:n0} concurrent jobs)") { Enabled = false });
                actions.DropDownItems.Add(new ToolStripMenuItem($"Billing: {_actionsInsights.Billing.DisplayText}") { Enabled = false });
                actions.DropDownItems.Add(new ToolStripMenuItem($"{_actionsInsights.RemainingIncludedMinutes:n0} included minutes remaining") { Enabled = false });
