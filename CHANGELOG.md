...
- Add an initial native Windows taskbar tray companion with configured repository status, local Git state, repository discovery and visibility controls, recent lists, discussions, activity, Actions queue/billing/runner, release, traffic, repository and account heatmap summaries, contribution totals, changelog summaries, GitHub navigation links, native preferences with named account profiles, GitHub App OAuth, account-scoped Credential Manager token storage, ETag cache, archive-backed issue/PR fallback, rate-limit diagnostics, Issue Navigator preview/copy/open actions, PR notification click-through, launch-at-login and update-check controls, installer packaging, Windows smoke/Crabbox validation scripts with test/smoke artifacts, and a captured Windows tray screenshot proof.
