...
- Add an initial native Windows taskbar tray companion with rich repository status rows, host-aware local Git state, filtered repository discovery with Preferences scan feedback, account-scoped repository visibility and repository move controls, tray and preferences repo display limit/sort/scope controls with top-level scope and sort switchers, Preferences and tray My repositories owner filtering, repository heatmap placement/window controls, main/repository menu customization with grouped repository submenus, independent local open/worktree and repository manage actions, and required support/exit controls, local branch switching, worktree creation, optional dirty file entries, configurable local fetch cadence, fetch/sync/rebase/reset actions, auto-sync success notifications, preferred terminal actions, and repository checkout, recent lists with issue/PR viewer, label, and engagement filters, discussions, activity, Actions queue/billing/cache/artifact-retention/runner summaries with monitored-owner and plan-tier controls, release, traffic, repository and account heatmap summaries, contribution totals, changelog summaries, host-aware GitHub navigation links, native preferences with named account profiles, tray account switching, GitHub App OAuth with stubbed loopback/token-exchange proof, token check/refresh controls, host-aware GitHub App installation and PAT creation links, account-scoped Credential Manager token storage and tray logout, account-scoped ETag cache with tray cache reset, host-aware archive-backed issue/PR fallback, copyable diagnostics with cache/archive/local Git/rate-limit state plus refresh, repository menu, local scan, Actions, clipboard, and PR-notification settings, REST/GraphQL rate-limit diagnostics, compact rate-limit tray tooltip state, debug/file logging controls with runtime log-file proof, Issue Navigator preview/copy/open actions with source-host URL, Actions run URLs, commit URLs and hashes, pasted-order, direct issue-number shorthand, unique repo-name shorthand, grouped repo-heading lists, same-line repository prose references, numbered repository headings with nested count summaries, line-bounded explicit repository prose series, mixed issue/PR explicit repository series, repository-scoped `gh-` shorthand, primary URL list shortcuts, leading triage-list reference priority, single repository-only list context, copied local project path context, normal prose repository context, heading-scoped prose references, contextual back references, compound shorthand, and commit hashes, chained/ranged shorthand, and singular/plural prose reference-list preservation plus optional clipboard reference monitoring, configurable account-scoped PR notification events including closed/reopened/merged state changes, first-enable baseline reset, and click-through with duplicate suppression, About window with project/support links, launch-at-login and manual/automatic update-check controls with architecture-aware Windows installer asset detection and copyable update diagnostics, installer packaging with Windows node trusted CI code signing, Windows CI x64/ARM64 layout, TRX/smoke artifact upload and validation manifest with publish-layout checks, pnpm Windows package/proof wrappers, Windows smoke/Crabbox validation scripts with local Git, account-profile, archive-fallback, diagnostics, runtime settings proof artifacts, and a captured Windows tray screenshot proof.
