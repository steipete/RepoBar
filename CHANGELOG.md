...
- Add an initial native Windows taskbar tray companion with rich repository status rows, local Git state, filtered repository discovery, visibility controls, repo display limit/sort controls, main/repository menu customization, local branch switching, worktree creation, and repository checkout, recent lists, discussions, activity, Actions queue/billing/cache/runner, release, traffic, repository and account heatmap summaries, contribution totals, changelog summaries, GitHub navigation links, native preferences with named account profiles, GitHub App OAuth, account-scoped Credential Manager token storage and tray logout, ETag cache, archive-backed issue/PR fallback, REST/GraphQL rate-limit diagnostics, Issue Navigator preview/copy/open actions plus optional clipboard reference monitoring, configurable PR notification events and click-through with duplicate suppression, launch-at-login and update-check controls with Windows installer asset detection, installer packaging, Windows smoke/Crabbox validation scripts with test/smoke artifacts, and a captured Windows tray screenshot proof.
