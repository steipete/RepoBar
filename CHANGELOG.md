...
- Add an initial native Windows taskbar tray companion with configured repository status, local Git state, repository discovery and visibility controls, recent lists, discussions, activity, Actions queue/billing/runner, release, traffic, heatmap, contribution, and changelog summaries, GitHub navigation links, native preferences with Credential Manager token storage, ETag cache, rate-limit diagnostics, PR notification click-through, launch-at-login and update-check controls, installer packaging, and Windows smoke/Crabbox validation scripts.
